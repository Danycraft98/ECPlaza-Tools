...
//var promise = import("net");

const httpProxy = require('http-proxy'),
    net = require('net'),
    url = require('url'),
    regexp_tld = require('./regexp-top-level-domain'),
...
function getHandler(options, proxy) {
    ...
    return function (req, res) {
        ...
        const isRequestedOverHttps = req.connection.encrypted || /^\s*https/.test(req.headers['x-forwarded-proto']),
            proxyBaseUrl = (isRequestedOverHttps ? 'https://' : 'http://') + req.headers.host;
...
exports.createServer = function createServer(options) {
    ...
    const proxy = httpProxy.createServer(httpProxyOptions),
        requestHandler = getHandler(options, proxy);
    let server;
